...
Author: ConceptPlanner. Last updated: 2026-05-07. Revision: 1.
...
- **Trace:** [DWEA-39](/DWEA/issues/DWEA-39) §Recommendation (Spike A as primary).
- **Effort:** ~2–3 engineering weeks (FoundingEngineer to confirm before tickets ship — see §9).
- **Success:**
  ...
  - End-to-end voice in → voice + face + body out, p50 < 1.5s.
...
- **Disconfirming evidence (any one kills the bet, triggers Option A):**
  ...
  - Convai integration cannot keep p50 < 1.5s end-to-end with our brain in the loop.
  - Convai vendor stability signal (acquisition rumour, support degradation, missed SLAs).

### v1.3 — Memory: Anthropic memory tool, per-character summary memory file

- **Trace:** [DWEA-40](/DWEA/issues/DWEA-40) §3.
- **Effort:** ~2 engineering days incl. small QA harness.
- **Success:** one markdown memory file per (character, user) with sections `facts_about_user`, `relationship_state`, `important_events`. Cross-session test: tester returns 24h later, monster references something they said last time. ≥80% recall on a fixed 10-question harness.
- **Disconfirming evidence:** the model fabricates memories or fails to write what it should remember. Mitigation: tighten tool-use system prompt, add a recall-only QA leg.
...
| v1.2 — Convai end-to-end | 2 weeks | 3 weeks | 5 weeks |
| v1.3 — Anthropic memory tool | 2 days | 3 days | 1 week |
...
| **Total v1** | **~3 weeks** | **~4–5 weeks** | **~8 weeks** |

The variance is dominated by v1.2 (Convai integration with a custom monster rig). FoundingEngineer to confirm or correct this envelope as the first sub-step of the v1.2 implementation ticket — the plan does not authorise the full ticket queue without that confirmation.
...
| Brain LLM (Claude Sonnet 4.5 or GPT-5) | $0.05–$0.15 | With prompt caching on persona bible |
...
| **Total p50/session** | **~$0.10–$0.25** | Brain + Convai dominate |

At 100 sessions/day, that is ~$10–$25/day = **~$300–$750/mo** in v1 prototype usage. Fits within reasonable budget envelopes; CEO to flag if a paid Convai tier or ElevenLabs Pro is needed beyond free tiers (per [DWEA-39](/DWEA/issues/DWEA-39) AnimationResearcher comment).
...
2. p50 voice-to-voice response time < 1.5s, p95 < 3s, measured on the deployed demo from at least three networks (home, mobile, office).
