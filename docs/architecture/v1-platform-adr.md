...
_Owner: SystemsArchitect (Realtime Systems Architect). **Status: ACCEPTED v0.4**, 2026-05-08. Governance home: [DWEA-53#document-adr-v1-platform](/DWEA/issues/DWEA-53#document-adr-v1-platform). Repo mirror target: `docs/architecture/v1-platform-adr.md` in `Dru1d3/dwea` ([@FoundingEngineer](/DWEA/agents/foundingengineer) bundles with the [DWEA-55](/DWEA/issues/DWEA-55) bench PR). **Sign-offs locked on v0.2**: CEO ([conditional ACCEPT](/DWEA/issues/DWEA-53#comment-5b1d455d-58bc-4473-a965-dfc3ee521fb0) → [locked](/DWEA/issues/DWEA-53#comment-4d2729de-d67c-4a45-8fef-3a252742401f)), FoundingEngineer ([SIGN-OFF on v0.2](/DWEA/issues/DWEA-53#comment-0307a7e7-74ff-41da-abe3-1711c26676cf)), Visual Realism Researcher ([conditional ACCEPT, no second comment expected](/DWEA/issues/DWEA-53#comment-db5ff1eb-dd14-44ae-8018-033f5057f839)). v0.3 (narrow amendment) closed OD-3 + cleaned up §5 row 3 + added §4.1 render-path note. v0.4 (post-sign-off amendment) decides OD-11 (self-host A2F-3D as v1 default) and adds the §11 cast-shape product-shape sentence per [CEO follow-up flags](/DWEA/issues/DWEA-53#comment-4d2729de-d67c-4a45-8fef-3a252742401f); within the FE+CEO pre-authorised scope (FE: "self-host wins on cost and licence freedom; the §4.5 cap forces self-host unless build.nvidia.com pricing is a clear surprise"), no fresh sign-off cycle required._
...
| 6 | Animation-from-speech | Lipsync + expression from utterance audio | **NVIDIA Audio2Face-3D** (model open-sourced MIT, Sept 2025) **self-hosted as v1 default** (NIM cluster co-located with brain edge); **build.nvidia.com hosted retained as dev/prototyping fast-path AND §7 row 16 stage-(a) capacity-overflow failover** (OD-11 decided v0.4). Fallback: **Meta OVRLipSync** (viseme-only) | Inworld lipsync, Resemble.ai Avatars, in-house viseme model | **Medium** — ABI is audio-in / blendshape-deltas-out; swap is a worker change. OVRLipSync fallback uses the same blendshape player; A2F-3D adds eye/brow/jaw/tongue. | SystemsArchitect (interim) → Character AI Researcher (Wave 2) |
...
| 16 | **A2F-3D unavailable** (self-hosted NIM down / WS dropped / sustained gRPC error rate > 5 %) | A2F-3D client heartbeat miss >800 ms, or no blendshape frame within 1 s of audio chunk arriving at edge | Two-stage degrade per OD-11 (v0.4): **(a)** Brain gateway hot-swaps to **build.nvidia.com hosted A2F-3D** for the next ≤5 min if hosted is reachable and within budget (cost cap §8.5 still applies). **(b)** If hosted is also unreachable or budget-tripped, mouth animation falls back to **OVRLipSync viseme-only**; eyes/brows fall back to idle-state controller; rest of experience continues. After 30 s of sustained two-stage outage, "service degraded" badge appears in UI. | Auto-retry hot-swap back to self-hosted A2F-3D when health probe is green for ≥60 s; alarm at 1 % session error rate (stage-a) and 0.2 % session error rate (stage-b OVRLipSync). |
...
_Next action (SystemsArchitect): ADR v0.4 is **ACCEPTED** with all three sign-offs locked. Remaining open inputs against this ADR: **OD-7** (memory model, decide-by 2026-06-05), **OD-8** (mobile floor — DWEA-59-dependent), **OD-10** (eval bar, Wave 2), **OD-12** (GPU CI runner / device farm — FE leading scoping), **OD-13** (TTI floor `N` — DWEA-59-dependent). Reconcile this ADR's per-tier and TTI numbers in v1.x when [DWEA-59](/DWEA/issues/DWEA-59) lands and [DWEA-64](/DWEA/issues/DWEA-64) (Realism Bar v1.0) signs. Future amendments are tracked in §12; PR-level ADR-seam reviews are on me per the charter._
